...
fun LibraryPlaylistsScreen(
    ...
    val triggerSearch = backStackEntry?.savedStateHandle?.getStateFlow("triggerSearch", false)?.collectAsStateWithLifecycle()
    val triggerCreatePlaylist = backStackEntry?.savedStateHandle?.getStateFlow("triggerCreatePlaylist", false)?.collectAsStateWithLifecycle()

    LaunchedEffect(triggerSearch?.value) {
        if (triggerSearch?.value == true) {
            isSearchActive = true
            backStackEntry?.savedStateHandle?.set("triggerSearch", false)
        }
    }
    LaunchedEffect(triggerCreatePlaylist?.value) {
        if (triggerCreatePlaylist?.value == true) {
            showCreatePlaylistDialog = true
            backStackEntry?.savedStateHandle?.set("triggerCreatePlaylist", false)
        }
    }
    ...
    val visibleResults = remember(
        ...
        buildList {
            // Only show auto-playlists (except Liked) if there are user created playlists
            val userPlaylistsExist = playlists.isNotEmpty()
